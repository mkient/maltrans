...
hbr_end_vm_contrast <- summary(hbr_end_m$contrasts, infer=TRUE)%>%as.data.frame()
hbr_end_vm_emmeans <- summary(hbr_end_m$emmeans, infer=TRUE)%>%as.data.frame()
...
df_endo_exo <- df_HBR%>%
  ...
  dplyr::summarise(sum = sum(Anopheles),mean = mean(Anopheles), sd=sd(Anopheles),se=sd(Anopheles)/sqrt(4))
df_endo_exo[,4:7][is.na(df_endo_exo[,4:7])] <- 0
